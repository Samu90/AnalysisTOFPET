...
void GetSpectrum(TTree* tree,TTree* treeCoinc, TH1D* histoCh1, TH1D* histoCh2, TH1D* histoCh3,Double_t MeanPedCh59, Double_t MeanPedCh315,Double_t MeanPedCh291){
  ...
  for(int i=0; i<treeCoinc->GetEntries(); i++){
    ...
    if(chIDCoinc[1]!=-9 && chIDCoinc[2]!=-9){histoCh2->Fill((energyCoinc[1]-MeanPedCh315+energyCoinc[2]-MeanPedCh291));}
  }
...
TF1* FitNaSpectrumCBBar(TH1D* Profile,Int_t* fitStatus ,Int_t chID){
  ...
  Profile->GetXaxis()->SetRangeUser(14,20); 
  peak1 = Profile->GetBinCenter(Profile->GetMaximumBin());
  Profile->GetXaxis()->UnZoom();   
  ...
  if(chID==59 ){
    ...
    *fitStatus=Profile->Fit(Form("SpectrumFit_%s", Profile->GetName()),"RM0");
    ...
    spectrum->SetRange(peak1-3,45);
      
    spectrum->SetParameter(0,7.9e4);
    spectrum->SetParameter(1,peak1);
    // spectrum->SetParameter(1,19);
    spectrum->SetParameter(2,2.2);
    // spectrum->SetParameter(3,max/5);
    //spectrum->SetParameter(4,0.82);
    spectrum->SetParameter(5,4000);
    spectrum->SetParameter(6,37);
    //spectrum->SetParameter(7,3.2);
    spectrum->SetParameter(7,1.9);
    spectrum->SetParameter(8,8000);
    //spectrum->SetParameter(9,peak2/1.18);
    spectrum->SetParameter(9,28);
    spectrum->SetParameter(10,4.6);
    spectrum->SetParameter(11,1.1);
    spectrum->SetParameter(12,-4e5);
    
    spectrum->SetParLimits(10,3.6,5);
...
int main(int argc, char* argv[] ){
  ...
  for(int i=0;i < NFilePhys;i++){
    ...
    HistoToWrite[0]=HistoCh59[i];
    HistoToWrite[1]=HistoCh315[i];

    fileFit = new TFile("Spectrum.root","RECREATE");
    HistoToWrite[0]->Write();
    HistoToWrite[1]->Write();
    fileFit->Save();
    fileFit->Close();
    ...
    gSystem->Exec("python FitSpectrum.py");

    spectraFile= TFile::Open("SpectrumFile.root","READ");
    spectraFile->cd();
    
    FitSpectrum[i][0]=(TF1*)spectraFile->Get("fTot_pixel");
    FitSpectrum[i][1]=(TF1*)spectraFile->Get("fTot_bar");

    for(int k=0; k<FitSpectrum[i][0]->GetNpar();k++){

      std::cout << i << ": "<< FitSpectrum[i][1]->GetParameter(k) << std::endl; 
    ...
    canvino->Divide(2,1);
    std::cout << "Address1:  " << FitSpectrum[i][0] << "    " << HistoCh59[i] <<std::endl;
    std::cout << "Address2:  " << FitSpectrum[i][1] << "    " << HistoCh315[i]<< std::endl;    
    std::cout << "Canvas:" << canvino << std::endl;
    
    canvino->cd(1);
    HistoCh59[i]->Draw();
    HistoCh59[i]->Fit(FitSpectrum[i][0]);
    FitSpectrum[i][0]->Draw("SAME");
    
    canvino->cd(2);
    HistoCh315[i]->Draw();
    HistoCh315[i]->Fit(FitSpectrum[i][1]); 
    FitSpectrum[i][1]->Draw("SAME");    
    
    canvino->SaveAs((DirData+"/Plot/EnergyTempCB/Partials/Canvas"+std::to_string(i)+".png").c_str());
    
    //    gSystem->Exec("rm Spectrum.root SpectrumFile.root");
  ...
  for(int i=0;i<NFilePhys;i++){
    ...
    Peak1Ch59[i]=FitSpectrum[i][0]->GetParameter(10);
    SigmaPeak1Ch59[i]=FitSpectrum[i][0]->GetParError(10);
    Peak2Ch59[i]=FitSpectrum[i][0]->GetParameter(15);
    SigmaPeak2Ch59[i]=FitSpectrum[i][0]->GetParError(15);

    Peak1Ch315[i]=FitSpectrum[i][1]->GetParameter(10);
    SigmaPeak1Ch315[i]=FitSpectrum[i][1]->GetParError(10);
    Peak2Ch315[i]=FitSpectrum[i][1]->GetParameter(15);
    SigmaPeak2Ch315[i]=FitSpectrum[i][1]->GetParError(15);
  ...
  */
  ...
  for(int i=0;i<NFilePhys;i++){
    ...
    MeanCh59P1[i] = FitSpectrum[i][0]->GetParameter(10);
    MeanCh315P1[i] = FitSpectrum[i][1]->GetParameter(10);
    SigmaCh59P1[i] = FitSpectrum[i][0]->GetParameter(11);
    SigmaCh315P1[i] = FitSpectrum[i][1]->GetParameter(11);

    ErrMeanCh59P1[i] = FitSpectrum[i][0]->GetParError(10);
    ErrMeanCh315P1[i] = FitSpectrum[i][1]->GetParError(10);
    ErrSigmaCh59P1[i] = FitSpectrum[i][0]->GetParError(11);
    ErrSigmaCh315P1[i] = FitSpectrum[i][1]->GetParError(11);
    
    MeanCh59P2[i] = FitSpectrum[i][0]->GetParameter(4);
    MeanCh315P2[i] = FitSpectrum[i][1]->GetParameter(4);
    SigmaCh59P2[i] = FitSpectrum[i][0]->GetParameter(5);
    SigmaCh315P2[i] = FitSpectrum[i][1]->GetParameter(5);

    ErrMeanCh59P2[i] = FitSpectrum[i][0]->GetParError(4);
    ErrMeanCh315P2[i] = FitSpectrum[i][1]->GetParError(4);
    ErrSigmaCh59P2[i] = FitSpectrum[i][0]->GetParError(5);
    ErrSigmaCh315P2[i] = FitSpectrum[i][1]->GetParError(5);
  ...
  */
  ...
  for(int i=0;i<NFilePhys;i++){
    ...
    gSystem->Exec( ("echo "+DirData+" \t" + std::to_string(FitCalib[1]->GetParameter(1)) + "\t"+std::to_string(FitSpectrum[0][1]->GetParameter(10))+ ">> ../LY.txt" ).c_str() );
