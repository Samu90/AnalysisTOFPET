...
void Analysis(){
  ...
  Int_t Nch=2;
  Int_t TimeBin=30,EnergyBin=100,TempBin=70;
  Double_t TMin=0,TMax=20000,EMin=0,EMax=100,TempMin=26,TempMax=36;
  ...
  Double_t X[TimeBin];
  std::vector<std::vector<Double_t> > Y(8, std::vector<Double_t>(TimeBin));
  //Y1->[0]:Y511ch59,[1]:EY511ch59,[2]Y1275ch59,[3]EY1275ch59
  //    [4]:Y511ch315,[5]:EY511ch315,[6]Y1275ch315,[7]EY1275ch315

  GetProfiles(HistoEnergyVsTime[0],HistoEnergyVsTime[1],X,Y);
  
   
  TGraphErrors* graph1Ch59 = new TGraphErrors(TimeBin,X,&Y[0][0],0,&Y[1][0]);
  TGraphErrors* graph2Ch59 = new TGraphErrors(TimeBin,X,&Y[2][0],0,&Y[3][0]);
  TGraphErrors* graph1Ch315 = new TGraphErrors(TimeBin,X,&Y[4][0],0,&Y[5][0]);
  TGraphErrors* graph2Ch315 = new TGraphErrors(TimeBin,X,&Y[6][0],0,&Y[7][0]);
      
  if(PrintVectorTGraph){
    for(int i=0;i<8;i++){
      for(int j=0;j<TimeBin;j++){
	std::cout<<X[j] <<"    " << Y[i][j]<< std::endl;
      }//chiudo for j
    }//chiudo for i
  }//chiudo if
